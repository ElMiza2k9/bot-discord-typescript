...
export default class Resolver {
	...
	public async getAlbum(id: string): Promise<LavalinkTrackResponse> {
		try {
			...
			const unresolvedAlbumTracks =
				spotifyAlbum?.tracks.items.map(track => this.buildUnresolved(track)) ?? []

			return this.buildResponse(
				'PLAYLIST_LOADED',
				this.autoResolve
					? ((await Promise.all(unresolvedAlbumTracks.map(x => x.resolve()))).filter(
							Boolean
					  ) as LavalinkTrack[])
	...
	public async getPlaylist(id: string): Promise<LavalinkTrackResponse> {
		try {
			...
			const unresolvedPlaylistTracks = spotifyPlaylist.tracks.items.map(x =>
				this.buildUnresolved(x.track)
			)

			return this.buildResponse(
				'PLAYLIST_LOADED',
				this.autoResolve
					? ((await Promise.all(unresolvedPlaylistTracks.map(x => x.resolve()))).filter(
							Boolean
					  ) as LavalinkTrack[])
	...
	public async getTrack(id: string): Promise<LavalinkTrackResponse> {
		try {
			...
			return this.buildResponse(
				'TRACK_LOADED',
				this.autoResolve
					? ([await unresolvedTrack.resolve()] as LavalinkTrack[])
					: [unresolvedTrack]
			)
		} catch (e) {
			return this.buildResponse(
	...
	private async getPlaylistTracks(spotifyPlaylist: SpotifyPlaylist): Promise<void> {
		...
		while (
			nextPage &&
			(this.playlistLoadLimit === 0 ? true : pageLoaded < this.playlistLoadLimit)
		) {
			// @ts-expect-error 2322
			const { body: spotifyPlaylistPage }: { body: SpotifyPlaylist['tracks'] } =
				await request.get(nextPage).set('Authorization', this.token)
	...
	): Promise<LavalinkTrack | undefined> {
		...
		if (lavaTrack) {
			if (this.client.options.useSpotifyMetadata) {
				Object.assign(lavaTrack.info, {
					...
					thumbnail: unresolvedTrack.info.thumbnail
				})
			}
	...
	private async retrieveTrack(
		unresolvedTrack: UnresolvedTrack
	): Promise<LavalinkTrack | undefined> {
		// console.log(unresolvedTrack)
		const params = new URLSearchParams({
			identifier: `ytmsearch:${unresolvedTrack.info.title} - ${
				unresolvedTrack.info.author
			} ${this.client.options.audioOnlyResults ? '' : ''}`
		})

		// @ts-expect-error 2322
		const { body: response }: { body: LavalinkTrackResponse<LavalinkTrack> } =
			await request
				.get(
					`http${this.node.secure ? 's' : ''}://${this.node.host}:${
						this.node.port
					}/loadtracks?${params.toString()}`
				)
				.set('Authorization', this.node.password)

		// console.log(`Debug: ${response.tracks[0].info.title} - ${response.tracks[0].info.uri}`);
	...
	private buildUnresolved(spotifyTrack: SpotifyTrack): UnresolvedTrack {
		...
		// const authors = makestr(spotifyTrack.artists.map((x) => x.name))

		return {
			info: {
				identifier: spotifyTrack.id,
				title: spotifyTrack.name,
				author: spotifyTrack.artists.map(x => x.name)[0],
	...
	private buildResponse(
		loadType: LavalinkTrackResponse['loadType'],
