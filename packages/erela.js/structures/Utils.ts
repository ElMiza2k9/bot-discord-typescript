...
export abstract class TrackUtils {
	...
	static validate(trackOrTracks: unknown): boolean {
		...
		return (
			(trackOrTracks[TRACK_SYMBOL] || trackOrTracks[UNRESOLVED_TRACK_SYMBOL]) === true
		)
	}
	 ...
	 * @param requester
	 */
	static build(data: any, requester?: User): Track {
		...
		try {
			const track: Track = {
				...
				thumbnail: data.info.uri.includes('youtube')
					? `https://img.youtube.com/vi/${data.info.identifier}/hqdefault.jpg`
					: data.info.thumbnail ||
