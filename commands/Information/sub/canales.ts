...
export default {
	do: async (client: Client, interaction: CommandInteraction) => {
		...
		const roles = await interaction.guild.channels.fetch()

		if (!roles || !roles.size) return interaction.followUp(`${ee.thong} No hay canales`)
		...
		const pages = chunk.map(i => {
			const embed = new MessageEmbed()
				...
				.setTitle(`<:channel:915638043124326450> Canales de ${interaction.guild.name}`)
