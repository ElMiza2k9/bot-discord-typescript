...
export default {
	do: async (client: Client, interaction: CommandInteraction) => {
		...
		if (findemoji.roles.cache.size >= 5)
			return interaction.followUp({
				embeds: [
					{
						description:
							'<:cross:909608584642437150> Ya he asignado este emoji a 5 roles diferentes',
						color: client.getcolor()
					}
				]
			})
		...
		if (role.id == interaction.guild.roles.everyone.id)
			return interaction.followUp({
				embeds: [
					{
						description:
							'<:cross:909608584642437150> Si deseas liberar un emoji usa `/emoji unlock | /emoji unlock-all`.',
						color: client.getcolor()
					}
				]
			})
		...
		if (role.position >= interaction.guild.me.roles.highest.position)
			return interaction.followUp({
				embeds: [
					{
						description:
							'<:cross:909608584642437150> No puedo gestionar este rol. Asigname un rol superior :)',
						color: client.getcolor()
					}
				]
			})
		...
		if (
			...
			return interaction.followUp({
				embeds: [
					{
						description:
							'<:cross:909608584642437150> Necesitas un rol superior para agregar este rol.',
						color: client.getcolor()
					}
				]
			})
		...
		if (findemoji.roles.cache.has(role.id))
			return interaction.followUp({
				embeds: [
					{
						description: '<:cross:909608584642437150> Este rol ya ha sido asignado.',
						color: client.getcolor()
					}
				]
			})

		findemoji.roles.add(role.id)

		return interaction.followUp({
			embeds: [
				{
					description: `<:check:909608584260751361> **¡${findemoji.name} ha sido asignado a ${role}!**\nRecuerda que al otorgar el rol a un miembro, este debe reiniciar Discord.`,
					color: client.getcolor()
				}
			]
		})
