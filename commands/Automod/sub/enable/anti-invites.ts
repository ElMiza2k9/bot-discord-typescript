...
export default {
	do: async (client: Client, interaction: CommandInteraction) => {
		...
		let message = data.automod.antiinvites.enabled
			...
			: `${client.config.emojis.check} Se activó el anti-invites`

		data.automod.antiinvites.enabled = true
		data.automod.antiinvites.action = action
		data.automod.antiinvites.ignoreAdmin = ignoreAdmin
